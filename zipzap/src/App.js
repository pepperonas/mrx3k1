...
function App() {
    ...
    const generateZipBomb = async () => {
        ...
        setDownloadUrl(null);
        setGenerationStatus('Starting zip bomb generation...');
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to generate zip bomb');
            }
            ...
            if (data.success) {
                setGenerationStatus(`Done! Created zip bomb at '${data.filePath}'`);
                setDownloadUrl(data.downloadUrl);
                setGenerationComplete(true);
            } else {
                throw new Error(data.message || 'Unknown error generating zip bomb');
            }
        } catch (error) {
                ...
                {!success ? (
                  ...
                  ${downloadUrl
                            ...
                            >
                                ...
                                {downloadUrl ? 'Download Zip' : 'Generating...'}
                            </button>
                        </div>
                    </div>
                )}
