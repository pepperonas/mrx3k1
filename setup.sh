...
# Dieses Skript erstellt ein komplett neues React-Projekt ohne Tailwind
# und konfiguriert ClipTune darin

GREEN='\033[0;32m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

echo -e "${BLUE}Erstelle neues ClipTune-Projekt ohne Tailwind${NC}"
echo "=================================================="

# 1. Erstelle ein komplett neues React-Projekt in einem temporären Verzeichnis
TEMP_DIR="cliptune"
echo "Erstelle neues Projekt in $TEMP_DIR..."

# Erstelle das Verzeichnis
mkdir -p $TEMP_DIR
cd $TEMP_DIR

# Initialisiere ein neues React-Projekt
echo "Initialisiere React-App (das kann einige Minuten dauern)..."
npx create-react-app . --use-npm

if [ $? -ne 0 ]; then
    echo "Fehler beim Erstellen der React-App."
    exit 1
fi

echo -e "${GREEN}✓${NC} React-App initialisiert"

# 2. ClipTune.js erstellen
echo "Erstelle ClipTune-Komponente..."

cat > src/ClipTune.js << 'EOL'
import React, { useState, useRef, useEffect } from 'react';

// Icons manuell definieren
const PlayIcon = () => (
...
const ClipTune = () => {
  ...
  const styles = {
    ...
    // Media query styles werden mit CSS Media Queries behandelt
  };
    ...
    <div style={styles.container} onDragOver={handleDragOver} onDrop={handleDrop}>
      ...
      <div style={styles.fileInput}>
        <label style={styles.fileButton}>
          <input type="file" accept="audio/*" style={styles.hiddenInput} onChange={handleFileSelect} />
          <UploadIcon />
          Datei öffnen
        </label>
        <span style={styles.dropHint}>oder Audiodatei hierher ziehen</span>
      </div>
      ...
      >
        <canvas ref={canvasRef} width={1000} height={150} style={{width: '100%', height: '100%'}} />
        {!audioBuffer && (
      ...
      <div style={styles.timeDisplay}>
        ...
        <div style={styles.timeText}>Auswahl: {formatTime(endMarker - startMarker)}</div>
      </div>

      <div style={{...styles.controlsGrid, '@media (min-width: 768px)': {gridTemplateColumns: '1fr 1fr'}}}>
        <div style={styles.controlPanel}>
          <h3 style={styles.controlTitle}>Wiedergabe</h3>
        ...
        <div style={styles.controlPanel}>
          <h3 style={styles.controlTitle}>Marker-Steuerung</h3>
          ...
          <div style={styles.markerRow}>
            ...
            <span style={styles.stepLabel}>Schrittweite:</span>
            <select
              value={stepSize.start}
              onChange={(e) => setStepSize({...stepSize, start: parseInt(e.target.value)})}
          ...
          <div style={styles.markerRow}>
            <span style={styles.markerLabel}>Ende:</span>
            <span style={styles.markerValue}>{formatTime(endMarker)}</span>
            <span style={styles.stepLabel}>Schrittweite:</span>
            <select
              value={stepSize.end}
              onChange={(e) => setStepSize({...stepSize, end: parseInt(e.target.value)})}
              style={styles.stepSelect}
            >
              <option value="10">10ms</option>
              <option value="100">100ms</option>
      ...
      <div style={styles.statusBar}>
        {audioFile ? `Geladen: ${audioFile.name}` : "Bereit. Öffnen Sie eine Audiodatei, um zu beginnen."}
      </div>
    </div>
  );
};
...
echo "Aktualisiere App.js..."

cat > src/App.js << 'EOL'
import React from 'react';
import ClipTune from './ClipTune';
import './App.css';
...
# 4. App.css aktualisieren
echo "Aktualisiere App.css..."
cat > src/App.css << 'EOL'
.App {
  width: 100%;
  height: 100vh;
...
echo -e "${GREEN}✓${NC} ClipTune-Projekt erfolgreich erstellt"
echo -e "\nUm die App zu starten, führe folgende Befehle aus:"
